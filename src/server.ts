...
function serverInstructions(
  ...
  const backgroundTasks =
    ` For non-trivial multi-step work, immediately start ${toolNames.backgroundTask} before the first edit or long command so a stalled ChatGPT turn always leaves a durable handoff. If the work has a clear 2-10 stage plan, persist it. Durable task identity follows the canonical source repository across managed worktrees while workspace_root remains the current execution checkout; Runtime will not switch execution roots while an active job or manual session is pinned. When a manually tracked long command finishes, clear active_session_id with null before moving the task to another worktree. Deterministic stages such as tests, build, verify, commit/push, or waiting for GitHub CI may include a local command; set auto_run=true when Runtime should run those stages and advance on successful process exit without asking the user to say continue. A GitHub CI stage can use a blocking command such as gh run watch ... --exit-status, so CI completion becomes the trigger. Runtime stops automatic advancement on command failure, orphaned outcome, or a stage without a command; ChatGPT remains the only reasoning/coding owner. Keep stage summaries short and command output in Runtime evidence. After a page stall, reconnect, worktree switch, or new ChatGPT conversation, call ${toolNames.backgroundTask} with action=status and no task_id once after opening the repo; Runtime will recover the latest task, including completed results and remaining stages.`;
