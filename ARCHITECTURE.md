...
Codex-facing commands use bounded event waits. Modern commands wait briefly for
short work to finish, then return a durable session with `retry_after_ms` so the
host does not busy-poll. Cached legacy ChatGPT calls keep a shorter first yield
to avoid holding older transport requests open.
